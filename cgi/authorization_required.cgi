...
if $cgi.include?("poll")
	if File.directory?($cgi["poll"])
		...
		$d << _("In order to proceed, you have to enter the password for user %{user}.") % {:user => "<code>#{$cgi["user"]}</code>"}
	...
	returnstr = _("Return to the Doodle homepage and create a new poll.")
