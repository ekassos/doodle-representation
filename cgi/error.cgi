...
if File.exists?($conf.errorlog)
	...
	subject = _("Bug in Doodle")

	$d << _("An error occurred while executing Doodle.<br/>Please send an error report, including your browser, operating system, and what you did to %{admin}.") % {:admin => "<a href='mailto:#{$conf.bugreportmail}?subject=#{CGI.escape(subject).gsub("+","%20")}&amp;body=#{CGI.escape(errormessagebody).gsub("+","%20")}'>#{$conf.bugreportmail}</a>"}
...
if $conf.auto_send_report && !known
	...
	%x{mail -s "Bug in Doodle" #{$conf.bugreportmail} < #{tmpfile}}
