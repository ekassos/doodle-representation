...
class Dudle
	...
	def inittabs
		@tabs = []
		@tabs << [_("Homepage"),@basedir]
		if is_poll?
			@tabs << ["",""]
			@tabs << [_("Poll Page"),"."]
			@tabs << [_("Change History"),"history.cgi"]
			@tabs << ["",""]
			@configtabs = [
				[_("Edit Times & Dates"),"edit_columns.cgi"],
				[_("Add Participants"),"invite_participants.cgi"],
				[_("Password Protection"),"access_control.cgi"],
				[_("Sharing"),"overview.cgi"]
			]
			@tabs += @configtabs
			@tabs << [_("Delete Poll"),"delete_poll.cgi"]
			@tabs << ["",""]
		else
			@tabs << [_("About Doodle"),"about.cgi"]
		end
		@tabs << [_("Appearance Settings"),"customize.cgi"]
	...
	def breadcrumbs
		...
		crumbs << "<a href='#{@basedir}'>" + _("Doodle Home") + "</a>"
		if is_poll?
			if @tab == "."
	...
	def initialize(params = {:revision => nil, :title => nil, :hide_lang_chooser => nil, :relative_dir => "", :load_extensions => true})
		...
		if is_poll?
			...
			@html = HTML.new("Doodle - #{@title} - #{@tabtitle}",params[:relative_dir])
			@html.add_html_head('<meta name="robots" content="noindex, nofollow" />')
			@html.header["Cache-Control"] = "no-cache"
		else
			@basedir = "."
			inittabs
			@title = params[:title] || "Doodle"
...
HEAD
	...
	def wizzard_nav
		...
		[[_("Previous page"),@wizzardindex == 0],
		 [_("Next page"),@wizzardindex >= @configtabs.size()-2],
		 [_("Finish setup"),@wizzardindex == @configtabs.size()-1]].each{|button,disabled|
			ret += <<READY
				<td>
...
READY
	...
	def wizzard_redirect
		[[_("Previous page"),@wizzardindex-1],
		 [_("Next page"),@wizzardindex+1],
		 [_("Finish setup"),@configtabs.size()-1]].each{|action,linkindex|
