...
if $cgi.include?("create_poll") && $cgi.include?("poll_url")
	...
	if polltitle == ""
		...
		if !(pollurl =~ /\A[\w\-_]*\Z/)
			createnotice = _("The custom URL may only contain letters, numbers, and dashes.")
...
unless $d.html.header["status"] == "REDIRECT"
	...
	$d << "<h2>"+ _("Create a new poll") + "</h2>"

	titlestr = _("Title")
	typestr = _("Type")
	timepollstr = _("Poll for finding the best time (e.&thinsp;g., schedule a meeting)")
	normalpollstr = _("Ordinary poll (e.&thinsp;g., vote for what is the best coffee variety)")
	customaddrstr = _("Set up a custom URL (optional)")
	customaddrhintstr = _("You may use letters, numbers, and dashes.")

	createstr = _("Create poll")
	$d << <<CREATE
