...
if $cgi.include?("ac_user")
	...
	if !(user =~ /^[\w]*$/)
		# add user
		usercreatenotice = "<div class='error'>" + _("Only letters and digits are allowed for the username.") + "</div>"
	elsif $cgi["ac_password0"] != $cgi["ac_password1"]
		usercreatenotice = "<div class='error'>" + _("The passwords you entered do not match.") + "</div>"
	else
...
$d.html << "<h2>" + _("Password Protection") + "</h2>" + "<p>" + _("Would you like to set up password protection for your poll? You can control who can change the proposed times and dates, or even vote.") + "</p>"
...
if acusers.empty? && $cgi["ac_activate"] != "Activate"
	...
	if acusers.empty?
		acstatus = ["blue",_("password protection will be activated when the admin login details have been configured.")]
		...
		acswitchbutton = "<div class='shorttextcolumn'>" + _("You have to remove all users before you can deactivate the password protection settings.") + "</div>"
	end


	admincreatenotice = usercreatenotice || _("Note: You will be asked for the password you have entered here after you click save!")

	user = ["admin",
	        _("The user ‘admin’ has access to both the voting and the configuration interface. Keep these details secure and do not share them with your invitees.")]

	createform = createform(user,admincreatenotice,acusers)
	if acusers.include?("admin")
		participantcreatenotice = usercreatenotice || ""
		user = ["participant",
	          _("The user ‘participant’ only has access to the voting interface. Give these details to your invitees.")]
	  createform += createform(user,participantcreatenotice,acusers)
	end

end

acstr = _("Password protection:")
$d.html << <<AC
